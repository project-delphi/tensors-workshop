...
NOTEBOOK = Path(__file__).resolve().parents[1] / 'notebooks/13-convolution-and-deconvolution.ipynb'


class ConvolutionFeedback(unittest.TestCase):
    def setUp(self):
        self.notebook = json.loads(NOTEBOOK.read_text())
        self.patch = np.arange(20.).reshape(4, 5)
        self.kernel = np.array([[1., 2.], [3., 4.]])
        self.scanline = np.array([1., 2., 4.])
        self.kernel_1d = np.array([1., 0., -1.])
        namespace = dict(np=np, signal=signal, patch=self.patch, sobel_x=self.kernel,
                         scanline=self.scanline, kernel_1d=self.kernel_1d)
    ...
    def test_rejects_each_shape_error(self):
        for i in range(4):
            ...
            with self.subTest(argument=i), self.assertRaisesRegex(AssertionError, 'output shape'):
                self.check(*args)
        args = self.args()
    ...
    def test_prediction_precedes_explanations(self):
        ...
        for explanation in ('p09-plain-idea', 'p13-cube', 'p09-modes'):
            self.assertLess(ids.index('p09-predict-kernel-flip'), ids.index(explanation))
