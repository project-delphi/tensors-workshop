...
class MatrixFeedback(unittest.TestCase):
    def setUp(self):
        ...
        self.X = np.array([[1., 0.], [1., 1.], [1., 2.], [1., 3.]])
        self.y = np.array([1., 2., 2., 4.])
        ...
        self.residuals = [np.linalg.norm(self.X @ b - self.y) for b in (self.normal, self.qr, ref)]
        self.errors = [np.linalg.norm(b - ref) / np.linalg.norm(ref) for b in (self.normal, self.qr)]

    def run_check(self, **changes):
        arguments = dict(X=self.X, y=self.y, beta_normal=self.normal,
                         beta_qr=self.qr, residuals=self.residuals,
...
class TensorFeedback(unittest.TestCase):
    def setUp(self):
        ...
        self.tensor = np.arange(1., 25.).reshape(2, 3, 4)
        self.cp_hat = self.tensor * .8
        self.tu_hat = self.tensor * .9
        self.counts = [2 * (2 + 3 + 4), 1 * 2 * 2 + 2 * 1 + 3 * 2 + 4 * 2]

    def run_check(self, **changes):
        arguments = dict(tensor=self.tensor, cp_rank=2, tucker_ranks=(1, 2, 2),
                         cp_hat=self.cp_hat, tu_hat=self.tu_hat,
                         parameter_counts=self.counts, relative_errors=[.2, .1])
        arguments.update(changes)
        with redirect_stdout(io.StringIO()):
            return self.check(**arguments)
    ...
    def test_rejects_swapped_errors_and_invalid_ranks(self):
        with self.assertRaisesRegex(AssertionError, "Relative error"):
            self.run_check(relative_errors=[.1, .2])
