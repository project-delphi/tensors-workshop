...
Work through linear-algebra-deep-learning — 14 short bilingual Colab notebooks (~2–3 hours total) covering exactly that material, vectors through PCA. This workshop moves fast through material it assumes you have reviewed.

Run notebook 00 in Colab before the session. It downloads the three CSV files below; a silent download failure leaves you stuck at Block 4 and Block 6, an hour in. If it fails, say so in Discord immediately.
...
569 real patients, 30 measurements from tumour cell images

(569, 30)

load_digits()

1797 real handwritten digits
...
Real histology and microscopy images
...
20,640 real housing districts from the 1990 US census

Pseudoinverse, least squares

NYC Taxi Trips

6,433 real taxi journeys in New York

Tensor factorization

Airline Passengers

144 months of real airline traffic, 1949–1960

Recursion, forecasting
...
An array of numbers with any number of axes
...
One direction along which data is arranged
...
Another word for axis, used in tensor theory
...
The size along each axis, as a tuple
...
The 3 colour values of one pixel

Unfolding

Rearranging a tensor into a matrix
...
Multiply and sum over a shared axis
...
Writing one tensor as a product of simpler ones
...
Gaussian elimination, saved for reuse
...
Perpendicular, unit-length directions
...
Directions that only get scaled (§2.7)
...
The most general matrix factorization (§2.8)
...
Compression to fewer features (§2.12)
...
"Inverse" when no true inverse exists (§2.9)
...
A "square root" of a covariance matrix, for building correlated data
...
Read the axes on real tensors. digits.images is (1797, 8, 8) and a photo is (512, 512, 3) — both order 3, but axis 0 counts whole images in one and rows of pixels in the other. digit_batch and video_patch are both (8, 8, 8). Before running anything, say what every axis counts.

Shuffle a batch vs shuffle time. Shuffling axis 0 is harmless for a batch — examples are independent, order carries no information — and destroys a video, where order is the information. The same operation, a completely different meaning. Chapter 2's notation has no concept of "order matters between elements." That is genuinely new today.

Batch clips of different lengths. Real videos have different frame counts, but a batch tensor is rectangular. Take three real clips of length 4, 7 and 5, pad them into one (3, 7, 135, 240, 3) order-5 batch, and carry a Boolean (3, 7) validity mask so valid.sum() == 16 — the padding stays visible instead of being averaged into the data.
...
Contract the original tensor against all factor matrices to get the core (Block 3).
...
The take-home notebooks below, and the two deep dives beyond them: matrix factorizations and tensor factorizations.

Further Reading — books, the seminal Tucker/CP/SVD papers, and tensorly, for going deeper than today's 195 minutes.
...
tensorly docs — Tucker and CP implementations, used in Block 6 and Appendix C. Created by Jean Kossaifi.
...
The original CP-versus-Tucker exercise has moved to
Notebook 13 — Tensor Factorizations.

Notebook 13 compares CP and Tucker at a matched parameter budget and extends
the discussion to Tensor Train and t-SVD.

🇪🇸 El ejercicio original de comparación entre CP y Tucker se trasladó al
Notebook 13 — Factorizaciones tensoriales.

Allí CP y Tucker se comparan con un presupuesto de parámetros equivalente
y el análisis se amplía a Tensor Train y t-SVD.

Appendix D — Take-Home: Cholesky Builds Correlated Data

Appendix E covers Cholesky as a solver — factor once, then solve cheaply many times. This appendix is the other half: Cholesky as a sampler. Feed a lower-triangular L with L @ L.T == Sigma some independent Gaussian noise and it hands back correlated draws with exactly that covariance — the mechanism behind every Monte Carlo simulation that needs correlated assets, sensors, or scenarios.
...
Factor once, solve many. Cholesky costs n³/3 once; each later solve is two triangular substitutions at O(n²). So m right-hand sides cost O(n³ + mn²), not O(mn³). np.linalg.inv(A) @ B is both slower and less accurate than factoring, and is never the right call.

The normal equations square the condition number, because κ(XᵀX) = κ(X)². QR's error scales with κ(X)·ε; the normal equations' with κ(X)²·ε. Same data, same objective, error squared.

Do not compute what you will throw away. A full SVD is O(mn·min(m,n)). If you want 20 components out of 1682, randomized SVD is O(mnk) and Lanczos is O(k·nnz(A)) — the gap between those is why large-scale recommenders are feasible at all.
...
The dedicated deep dive is
Notebook 13 — Tensor Factorizations.

A tensor decomposition is not only a compression technique. Each decomposition
makes a different assumption about which structure in the data should be kept.
...
One low-dimensional subspace per mode plus a core
...
Chain of small cores

approximately O(N · I · r²)
...
depends on retained tubal rank

Order-3 tensors with a meaningful third mode

🇪🇸 Cada método conserva una estructura distinta. CP busca componentes
individuales, Tucker permite un rango diferente por modo, Tensor Train evita
que un núcleo de orden alto crezca exponencialmente y t-SVD conserva la
estructura del tercer modo mediante FFT.

Why not flatten first?

Flattening preserves the numerical entries but can hide the meaning carried by
separate tensor modes.

The synthetic fluorescence-unmixing example makes the difference measurable:

CP recovers the true component amounts with correlation 1.00.

Flatten-then-SVD reaches only mean |corr| ≈ 0.536.

The SVD representation can produce negative amount directions even though
physical concentrations cannot be negative.

The example is synthetic by design; it isolates the structural question without
requiring a new dataset download.

🇪🇸 Aplanar no es necesariamente incorrecto. El problema aparece cuando
fusionamos dos modos cuyo significado separado era precisamente la información
que queríamos interpretar.

Fair comparison: CP versus Tucker

Do not compare CP rank R directly with Tucker rank (R, R, R).

Those two representations store different numbers of parameters.

A fair experiment is:

choose a CP rank;

count its stored parameters;

find Tucker ranks with approximately the same parameter budget;

compare reconstruction error and interpretability.

Notebook 13 performs this comparison on the real NYC taxi tensor

pickup borough × dropoff borough × hour

and uses the workshop's pinned storm clip as a second tensor-shaped example.

Exercise 1 — equal parameter budget

# TODO
# 1. Choose a CP rank R for the taxi tensor.
# 2. Count CP parameters: R * sum(T.shape).
# 3. Find Tucker ranks with approximately the same parameter count.
# 4. Compare relative reconstruction error.
# 5. Explain which factors are easier to interpret.

Why Tensor Train matters as order grows

For a dense order-N tensor with equal mode size I:

dense storage = I^N

With fixed TT bond rank r, storage is approximately:

TT storage = O(N · I · r²)

The dense representation grows exponentially with tensor order. TT grows
linearly in N when I and r are fixed.

CP storage is also linear in order when a fixed global rank is assumed; TT's
practical advantage is that it represents high-order interactions through local
bond ranks rather than one exponentially growing Tucker core.

Exercise 2 — storage versus tensor order

# TODO
# Fix I and TT bond rank r.
# Increase tensor order N.
# Plot dense storage I**N against TT storage ~ N*I*r**2.
# Explain the different growth rates.

Tensor decomposition inside neural networks

A dense convolution kernel with shape

3 × 3 × 512 × 512

contains 2,359,296 weights.

At a 14 × 14 feature map it requires about 462 million multiply-adds.

A CP factorization with rank 64 stores

64 × (3 + 3 + 512 + 512) = 65,920 weights

which is 35.8× fewer weights.

The factorized convolution can be implemented as four skinny convolutions in
sequence:

1×1 → 3×1 → 1×3 → 1×1

For a transformer output matrix W_O with shape 4096 × 4096, the dense matrix
stores 16,777,216 weights. A TT-matrix representation with rank 16 stores
34,816 weights, or about 481.9× fewer.

The practical workflow is:

train → compress → fine-tune

Compression is useful only if the downstream task remains accurate enough.

Exercise 3 — choose rank from an error requirement

# TODO
# Build the local synthetic convolution kernel from Notebook 13.
# Evaluate several CP ranks.
# Find the smallest rank whose reconstruction error is below a chosen threshold.
# Report the rank, error, stored weights and compression ratio.

The decision rule

Choose the decomposition from the structure you need to preserve, then choose
the rank from the loss you can afford.

🇪🇸 Elige primero el método según la estructura que necesitas conservar.
Después elige el rango según el error, almacenamiento o coste que puedes
aceptar.

For the full interactive treatment — method chooser, measured timing,
matched-budget CP/Tucker comparison, downstream compression and budget widget —
continue with
Notebook 13 — Tensor Factorizations.
