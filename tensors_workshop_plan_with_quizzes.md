...
The three photographs above are cubes, one per byte, on the [image tensor visualizer](interactive/tensor-visualizer.html?lang=en). Transpose NHWC to NCHW and watch the shape and the strides permute while the picture stays put; then compare with reshape and watch it break.
...
4. The recommender samples 8 frames out of 900. Which operation from section 03 does that, and what is lost?
...
The audio tensor stage carries the other two factorizations in this table, both measured rather than quoted. [Low rank](interactive/voice-stage.html?lang=en#lowrank) factors Appendix E's own `(513, 465)` matrix in your browser and rebuilds the SNR curve rung by rung as it goes, so the wait *is* the cost this section keeps asking about. [Parts you can name](interactive/voice-stage.html?lang=en#nmf) fits NMF to the same kind of matrix and prints its error beside the truncated SVD's at the same rank — worse, as Eckart–Young says it must be — then lets you solo one component and hear what that fraction of a percentage point bought.
...
3. Contract the original tensor against all factor matrices to get the core (section 06).
...
Each of those moves is a picture on the [audio tensor stage](interactive/voice-stage.html?lang=en), computed in the browser from this same recording. It cuts [one window](interactive/voice-stage.html?lang=en#frame) out of the array and shows the taper being applied to it; it [asks that one window which frequencies are in it](interactive/voice-stage.html?lang=en#spectrum), drawing all 1024 bins with the mirror half greyed out, which is why the matrix below has 513 rows and not 1024; and it [hops the window along](interactive/voice-stage.html?lang=en#window) so that each stop is one column of the `(513, 465)` you are about to factor. Play the rebuilt signal there with the overlap set to none and you can hear why the windows have to overlap. The truncation itself is the [low rank](interactive/voice-stage.html?lang=en#lowrank) picture: it adds the same 5 dB of noise, factors the matrix while you wait, and rebuilds this table's curve point by point rather than quoting it.
...
4.949 seconds at 48 kHz gives a `(513, 465)` STFT, so full rank is 465. The noisy input measures **5.00 dB**, as constructed:
