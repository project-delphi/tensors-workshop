...
One notebook per workshop section, numbered to match
[the section table](https://project-delphi.github.io/tensors-workshop) exactly,
plus take-home deep dives listed on the
[notebooks page](https://project-delphi.github.io/tensors-workshop/notebooks.html#going-further).
...
Deep dives are handed out as take-home material after the session. Each is declared
...
16 connects the matrix views of PCA with classification and clustering on real
Landsat patches, then fits multilinear features and explores a synthetic spiked
tensor. Its three animations are scientific diagrams generated by
`scripts/gen_pca_gifs.py`; the other notebooks use `scripts/gen_cube_gifs.py`.

17 and 18 are self-contained NumPy/PyTorch labs on multi-head attention and
SVD feature compression. They use synthetic data, embedded assertions and
pausable widgets. Their three GIFs apiece are generated from the notebook
...
Notebook 00 begins with an entry check, 12 with an exit check; 13 onward are
...
not to run anything. The notebooks the deps table calls network-free still compute
...
the notebooks use plots and interactive widgets. Both ship
with Colab, so their absence only shows up locally. `torch` is also included for the attention and compression labs.
`imageio`, `tensorly` and `pyttb` are not, because the notebooks that need them install them themselves.

`scikit-learn` and `scikit-image` ship the tumour data, the digits and the
photographs. Some notebooks use only bundled data; others download data or
install packages. See the generated requirements table on the
[notebooks page](https://project-delphi.github.io/tensors-workshop/notebooks.html#what-each-notebook-needs).
