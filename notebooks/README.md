...
plus four take-home deep dives, numbered 13–16, that are **not** sections.
...
Four notebooks are deep dives, handed out as take-home material after the
session: `13-convolution-and-deconvolution.ipynb`,
`14-cp-factorization.ipynb`, `15-generalized-cp.ipynb` and
`16-pca-from-tensors.ipynb`. Each is declared
...
Notebook 00 begins with an entry check, 12 with an exit check; 13–16 are
...
every notebook plots, and every one now builds widgets — the sixteen that
...
twelve fetch something the first time they run — see the table on the
