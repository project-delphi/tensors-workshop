...
format:
  revealjs:
    ...
    progress: true
    # The audience-facing timer and the section breadcrumb. One shared
    # file for both decks; it picks its language off <html lang>.
...
## Nothing here is invented {#all-the-data-here-is-real .data-slide}
...
[NYC taxi trips](https://github.com/mwaskom/seaborn-data)
...
[Airline passengers](https://github.com/mwaskom/seaborn-data)

144 months, 1949–60 · §08
:::

::: {.ds}
![](../../images/ds-digits.png){fig-alt="Eight handwritten digits from scikit-learn's load_digits, each 8 by 8 pixels, shown unsmoothed so the individual pixels stay visible."}

[Handwritten digits](https://scikit-learn.org/stable/datasets/toy_dataset.html#optical-recognition-of-handwritten-digits-dataset)
...
[Breast tumours](https://scikit-learn.org/stable/datasets/toy_dataset.html#breast-cancer-wisconsin-diagnostic-dataset)

569 patients, 30 measures · §03 §11
:::

::: {.ds}
![](../../images/ds-cell.jpg){fig-alt="A single cell floating in saline, imaged as a quantitative phase map recovered from a digital hologram."}

[Cell microscopy](https://scikit-image.org/docs/stable/api/skimage.data.html#skimage.data.cell)
...
[Storm footage](https://commons.wikimedia.org/wiki/File:Tormenta_en_l%27Almadrava.webm)

24 s at 960×540 · §05
:::

::: {.ds}
![](../../images/ds-audio.png){fig-alt="The waveform of a five-second voice recording, amplitude against time, with bursts of speech separated by quieter gaps."}

[Voice recording](https://github.com/pdx-cs-sound/wavs)
...
[California housing](https://github.com/ageron/handson-ml2/blob/master/datasets/housing/housing.csv)
...
[Stained histology](https://scikit-image.org/docs/stable/api/skimage.data.html#skimage.data.immunohistochemistry)
...
[Photographs](https://scikit-image.org/docs/stable/api/skimage.data.html) · §01 §06 §09 §10
:::
:::

Real data breaks in ways random numbers never do — missing values, mismatched
scales, pixels that never change. **Finding that is the work.**
...
::: {.pace-badge}
[[⏱️]{.pace-icon}**{{< var sections.s00.minutes >}} min** allocated]{.pace-duration}

[Start: {{< var sections.s00.start >}} · End: {{< var sections.s00.end >}}]{.pace-window}
:::
...
The same cell installs the ffmpeg backend §05 needs and pings the video host,
so a missing decoder surfaces now rather than two hours in.
...
::: {.pace-badge}
[[⏱️]{.pace-icon}**{{< var sections.s01.minutes >}} min** allocated]{.pace-duration}

[Start: {{< var sections.s01.start >}} · End: {{< var sections.s01.end >}}]{.pace-window}
:::
...
::: {.pace-badge}
[[⏱️]{.pace-icon}**{{< var sections.s02.minutes >}} min** allocated]{.pace-duration}

[Start: {{< var sections.s02.start >}} · End: {{< var sections.s02.end >}}]{.pace-window}
:::

## Same shape, different axes

`digits.images` is `(1797, 8, 8)` and a photo is `(512, 512, 3)` — both order 3,
but axis 0 counts **images** in one and **rows of pixels** in the other. In the
notebook, `digit_batch` and `video_patch` are *both* `(8, 8, 8)`: one stacks
independent examples, the other stacks ordered moments.

. . .

**Shuffling axis 0** is harmless for a batch — the examples are independent —
and **destroys** a video, where order *is* the information. Same code, opposite
meaning. Chapter 2's notation has no concept of "order matters between
elements"; that is genuinely new today.

## Ragged clips need a mask

Real videos have different lengths, but a batch tensor is rectangular. The
notebook takes three real clips of length 4, 7 and 5, pads them into one
`(3, 7, 135, 240, 3)` order-5 batch, and carries a Boolean `(3, 7)` **validity
mask** — so `valid.sum() == 16` and the padding stays visible instead of being
...
::: {.pace-badge}
[[⏱️]{.pace-icon}**{{< var sections.s03.minutes >}} min** allocated]{.pace-duration}

[Start: {{< var sections.s03.start >}} · End: {{< var sections.s03.end >}}]{.pace-window}
:::
...
::: {.pace-badge}
[[⏱️]{.pace-icon}**{{< var sections.s04.minutes >}} min** allocated]{.pace-duration}

[Start: {{< var sections.s04.start >}} · End: {{< var sections.s04.end >}}]{.pace-window}
:::
...
::: {.pace-badge}
[[⏱️]{.pace-icon}**{{< var sections.s05.minutes >}} min** allocated]{.pace-duration}

[Start: {{< var sections.s05.start >}} · End: {{< var sections.s05.end >}}]{.pace-window}
:::
...
::: {.pace-badge}
[[⏱️]{.pace-icon}**{{< var sections.s06.minutes >}} min** allocated]{.pace-duration}

[Start: {{< var sections.s06.start >}} · End: {{< var sections.s06.end >}}]{.pace-window}
:::
...
::: {.pace-badge}
[[⏱️]{.pace-icon}**{{< var sections.s07.minutes >}} min** allocated]{.pace-duration}

[Start: {{< var sections.s07.start >}} · End: {{< var sections.s07.end >}}]{.pace-window}
:::
...
::: {.pace-badge}
[[⏱️]{.pace-icon}**{{< var sections.s08.minutes >}} min** allocated]{.pace-duration}

[Start: {{< var sections.s08.start >}} · End: {{< var sections.s08.end >}}]{.pace-window}
:::
...
::: {.pace-badge}
[[⏱️]{.pace-icon}**{{< var sections.s09.minutes >}} min** allocated]{.pace-duration}

[Start: {{< var sections.s09.start >}} · End: {{< var sections.s09.end >}}]{.pace-window}
:::
...
::: {.pace-badge}
[[⏱️]{.pace-icon}**{{< var sections.s10.minutes >}} min** allocated]{.pace-duration}

[Start: {{< var sections.s10.start >}} · End: {{< var sections.s10.end >}}]{.pace-window}
:::

## What different factorizations buy you {.factorization-ladder}

> A factorization picks the atoms, the rule for combining them, and the
> property you buy. The object never changes — only which question becomes
> trivial.
...
$AB^{\top} = (AM)(BM^{-\top})^{\top}$ — generic factors are not unique
without additional structure or conventions.
:::
...
Essentially unique up to permutation and compensating scale, under suitable
conditions — sufficient: $k_A + k_B + k_C \ge 2R + 2$.
:::
:::

::: {.callout-tip}
Matrix multiplication is itself a tensor; its CP rank is the multiplication
count. The $2 \times 2$ case has rank 7, not 8 — that is Strassen. At the top
of the ladder, the factorization is the algorithm.
:::
...
::: {.pace-badge}
[[⏱️]{.pace-icon}**{{< var sections.s11.minutes >}} min** allocated]{.pace-duration}

[Start: {{< var sections.s11.start >}} · End: {{< var sections.s11.end >}}]{.pace-window}
:::
...
**Five take-homes** in [notebook 11]({{< var repo.colab_base >}}/11-wrap-up-and-take-homes.ipynb):
the PCA scaling trap, attention as two contractions, CP compared to Tucker,
building correlated portfolios with Cholesky, and denoising a real voice
recording by rank reduction.
